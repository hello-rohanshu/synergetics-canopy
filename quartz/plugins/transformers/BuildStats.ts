...
//
// Both are replaced with static text at build time, before markdown parsing.
// No client JS, no fetch, no nav-event race — values are frozen into the HTML
// at `npx quartz build` and only change on the next build.
...
export const BuildStats: QuartzTransformerPlugin = () => {
  return {
    ...
    textTransform(_ctx, src) {
      ...
      // Only bother reading files if a marker is actually present in this file.
